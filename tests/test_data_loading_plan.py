        ...
        with self.assertLogs("fedbiomed", logging.DEBUG) as captured:
            ...
            with self.assertRaises(FedbiomedLoadingBlockValueError):
                dlb5.deserialize(
                    {
                        **serialized,
                        "loading_block_class": "Wrong._format.__*$class.name",
                    }
                )
                self.assertEqual(
                    captured.output[-1],
            ...
            with self.assertRaises(FedbiomedLoadingBlockValueError):
                dlb5.deserialize(
                    {**serialized, "loading_block_module": "9Wrong.format.module.name"}
                )
                self.assertEqual(
                    captured.output[-1],
                    "CRITICAL:fedbiomed:FB614: data loading block error: "
            ...
            with self.assertRaises(FedbiomedLoadingBlockValueError):
                dlb5.deserialize(
                    {**serialized, "dlb_id": "serialized_dlb_wrong-format-uuid"}
                )
                self.assertEqual(
                    captured.output[-1],
                    "CRITICAL:fedbiomed:FB614: data loading block error: "
...
class TestDataLoadingPlan(unittest.TestCase):
    def setUp(self):
        ...
        # patchers
        self.patcher_infer_dataset = patch(
            "fedbiomed.common.dataloadingplan.DataLoadingPlan.infer_dataset_type",
    ...
    def test_data_loading_plan_01_interface(self):
        ...
        self.assertDictEqual(
            self.dlb1.data,
            dlp[LoadingBlockTypesForTesting.LOADING_BLOCK_FOR_TESTING].data,
        )
        self.assertDictEqual(
            self.dlb2.data,
            dlp[LoadingBlockTypesForTesting.OTHER_LOADING_BLOCK_FOR_TESTING].data,
        )

        it = iter(dlp.items())
        first_key, first_dlb = next(it)
        self.assertEqual(
            first_key, LoadingBlockTypesForTesting.LOADING_BLOCK_FOR_TESTING
        )
        self.assertDictEqual(self.dlb1.data, first_dlb.data)
        second_key, second_dlb = next(it)
        self.assertEqual(
            second_key, LoadingBlockTypesForTesting.OTHER_LOADING_BLOCK_FOR_TESTING
        )
        ...
        self.assertIn(
            LoadingBlockTypesForTesting.LOADING_BLOCK_FOR_TESTING.value, str_repr
        )
        self.assertIn(
            LoadingBlockTypesForTesting.OTHER_LOADING_BLOCK_FOR_TESTING.value, str_repr
        )
    ...
    def test_data_loading_plan_02_serialize_and_deserialize(self):
        ...
        self.assertNotIn(
            LoadingBlockTypesForTesting.OTHER_LOADING_BLOCK_FOR_TESTING, dlp2
        )
        ...
        self.assertIn(
            serialized_loading_blocks[0]["dlb_id"],
            serialized_dlp["loading_blocks"].values(),
        )
        self.assertIn(
            serialized_loading_blocks[1]["dlb_id"],
            serialized_dlp["loading_blocks"].values(),
        )
        ...
        with self.assertRaises(FedbiomedDataLoadingPlanError):
            dlp_metadata, dlbs_metadata = dlp.serialize()
            dlp_metadata["key_paths"][
                LoadingBlockTypesForTesting.LOADING_BLOCK_FOR_TESTING.value
    ...
    def test_data_loading_plan_03_mixin_functionality(self):
        ...
        apply_1 = tp.apply_dlb(
            "my default", LoadingBlockTypesForTesting.LOADING_BLOCK_FOR_TESTING
        )
        self.assertEqual(apply_1, "my default")
        apply_2 = tp.apply_dlb(
            "other default", LoadingBlockTypesForTesting.OTHER_LOADING_BLOCK_FOR_TESTING
        )
        ...
        apply_2 = list(
            tp.apply_dlb(
                "other default",
                LoadingBlockTypesForTesting.OTHER_LOADING_BLOCK_FOR_TESTING,
            )
        )
        ...
        class MyDataset(DataLoadingPlanMixin):
            ...
            def test_mapper(self):
                ...
                return self.apply_dlb(
                    orig_key, LoadingBlockTypesForTesting.TESTING_MAPPER, orig_key
                )
