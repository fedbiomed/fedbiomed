...
class TestIpython(unittest.TestCase):
    ...
    def test_utils_01_is_ipython(self):
        ...
        with patch.object(
            fedbiomed.common.ipython, "get_ipython", create=True
        ) as mock_get_ipython:
            mock_get_ipython.side_effect = TestIpython.ZMQInteractiveShell
            result = fb_ipython.is_ipython()
            self.assertTrue(
                result,
                f"`is_python` has returned {result}, while the expected is `True`",
            )
            ...
            self.assertFalse(
                result,
                f"`is_python` has returned {result}, while the expected is `False`",
            )
