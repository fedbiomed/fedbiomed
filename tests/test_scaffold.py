...
class TestScaffold(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.zero_model = copy.deepcopy(
            self.model
        )  # model where all parameters are equals to 0
        self.replies = {}
            ...
            self.replies.update(
                {
                    node_id: {
                        "node_id": node_id,
                        "optimizer_args": {
                            "lr": {k: 0.1 for k in self.model.state_dict().keys()}
                        },
                    }
                }
            )
    ...
    def assert_coherent_states(self, agg: Scaffold) -> None:
        ...
        for key, val in agg.global_state.items():
            avg = sum(states[key] for states in agg.nodes_states.values()) / len(
                agg.nodes_states
            )
        ...
        for node_id in self.node_ids:
            self.assertTrue(
                all(
                    (
                        agg.nodes_deltas[node_id][key]
                        == agg.nodes_states[node_id][key] - val
                    ).all()
                    for key, val in agg.global_state.items()
                )
            )
    ...
    def test_1_init_correction_states(self):
        ...
        agg = Scaffold(server_lr=1.0, fds=self.fds)
        ...
        self.assertTrue(
            all((agg.global_state[key] == 0).all() for key in agg.global_state)
        )
        ...
        for node in self.node_ids:
            ...
            self.assertTrue(
                all((agg.nodes_states[node][key] == 0).all() for key in global_model)
            )
            self.assertTrue(
                all((agg.nodes_deltas[node][key] == 0).all() for key in global_model)
            )
    ...
    def test_2_update_correction_state_all_nodes(self):
        ...
        agg.nodes_lr = {
            k: {layer: 1 for layer in self.model.state_dict().keys()}
    ...
    def test_3_update_correction_state_single_node(self):
        ...
        agg.init_correction_states(self.model.state_dict())
        agg.nodes_lr = {
            k: {layer: 1.0 for layer in self.models[k].keys()} for k in self.node_ids
        }
        # Test when a single client has non-zero-updates after 4 steps.
        updates = {
            key: torch.rand_like(val) for key, val in self.model.state_dict().items()
        }
        ...
        for node_id in self.node_ids[1:]:
            self.assertTrue(
                all(
                    (agg.nodes_states[node_id][key] == 0.0).all()
    ...
    def test_4_aggregate(self):
        ...
        training_plan.get_model_params = MagicMock(
            return_value=Linear(10, 3).state_dict()
        )

        agg = Scaffold(server_lr=0.2, fds=self.fds)
        n_round = 0

        weights = {node_id: 1.0 / self.n_nodes for node_id in self.node_ids}
    ...
    def test_6_create_aggregator_args(self):
        ...
        for node_id in self.node_ids:
            ...
            self.assertDictEqual(
                agg_args[node_id]["aggregator_correction"], agg.nodes_deltas[node_id]
            )
        ...
        for node_id in self.node_ids:
            self.assertTrue(agg_args[node_id].get("aggregator_name", False))
            self.assertDictEqual(
                agg_args[node_id].get("aggregator_correction"),
                agg.nodes_deltas[node_id],
            )
    ...
    def test_7_save_state_breakpoint(self, uuid_patch):
        ...
        self.assertEqual(
            save_patch.call_count, 2, f"'Serializer.dump' should be called 2 times"
        )
    ...
    def test_9_set_nodes_learning_rate_after_training(self):
        ...
        lr = {"layer-1": 0.1, "layer-2": 0.2, "layer-3": 0.3}
...
class TestIntegrationScaffold(unittest.TestCase):
    ...
    def test_1_aggregate_and_training_plan(self):
        ...
        for share_persistent_buffers_option in share_persistent_buffers_options:
            ...
            training_args = TrainingArgs(
                {"share_persistent_buffers": share_persistent_buffers_option},
                only_required=False,
            )
            ...
            local_models = {
                node_id: copy.deepcopy(tp.after_training_params())
                for i, node_id in enumerate(self.node_ids)
            }
            ...
            for node_id in self.node_ids:
                ...
                self.assertDictEqual(
                    scaffold.nodes_lr[node_id], tp.optimizer().get_learning_rate()
                )
    ...
    def test_2_bug_977_key_error(self):
            ...
            replies[0].update(
                {
                    node_id: {
                        "node_id": node_id,
                        "optimizer_args": {"lr": tp.optimizer().get_learning_rate()},
                    }
                }
            )
        ...
        with tempfile.TemporaryDirectory() as tmp_path:
            ...
            loaded_scaffold = Scaffold(server_lr=0.00123)
        ...
        for k_g, v_g in loaded_scaffold.global_state.items():
            ...
            self.assertTrue(
                torch.isclose(
                    v_g * len(self.node_ids),
                    torch.mean(
                        torch.stack(
                            [
                                loaded_scaffold.nodes_states[node_id][k_g]
                                for node_id in self.node_ids
                            ]
                        )
                    ),
