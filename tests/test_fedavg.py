...
class TestFedaverage(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.models = {
            f"node_{i}": copy.deepcopy(self.model).state_dict() for i in range(4)
        }
    ...
    def test_fed_average_02_sklearn_sgd_t1(self):
        ...
        model_params = {
            "node_1": {"coef_": np.array([3, 8, 8, 3, 1]), "intercept_": np.array([4])},
            "node_2": {
                "coef_": np.array([0.4, 1.6, 2, 1, 0.1]),
                "intercept_": np.array([1]),
            },
            "node_3": {"coef_": np.array([2, 5, 5, 3, 1]), "intercept_": np.array([6])},
        }
        weights = {"node_1": 0.2, "node_2": 0.2, "node_3": 0.6}

        aggregated_params = self.aggregator.aggregate(model_params, weights)
        self.assertTrue(
            np.allclose(
                aggregated_params["coef_"], np.array([1.88, 4.92, 5.0, 2.6, 0.82])
            )
        )
    ...
    def test_fed_average_03_sklearn_sgd_t2(self):
        ...
        weights = {"node_1": 0.27941176470588236, "node_2": 0.7205882352941176}

        model_params = {
            "node_1": {
                "coef_": np.array(
                    [-0.02629813, 0.04612957, -0.00321454, 0.08003535, 0.30818439]
                ),
                "intercept_": np.array([0.161345]),
            },
            "node_2": {
                "coef_": np.array(
                    [-0.02782622, 0.0145883, -0.01471519, -0.03673147, 0.45426254]
                ),
        ...
        self.assertTrue(
            np.allclose(
                aggregated_params["coef_"],
                np.array(
                    [-0.02739925, 0.0234013, -0.01150177, -0.00410545, 0.41344659]
                ),
            )
        )
        self.assertTrue(
            np.allclose(aggregated_params["intercept_"], np.array([0.04178598]))
        )
    ...
    def test_fed_average_04_save_state_breakpoint(self):
        ...
        self.assertDictEqual(
            expected_state, state, "State of FedAvg has not been saved correctly"
        )
    ...
    def test_fed_average_06_order_of_weight_and_model_params(self):
        ...
        model_params = {
            "node_02a6e376": {"coef_": np.array([0.0])},
            "node_7ea1c779": {"coef_": np.array([10.0])},
        }
        agg_params = self.aggregator.aggregate(
            model_params=model_params, weights=weights
        )
        self.assertEqual(agg_params["coef_"][0], 9.0)

        # test if missing node id triggers exception
        model_params["node_123abc"] = {"coef_": np.array([5.0])}

        with self.assertRaises(FedbiomedAggregatorError):
            self.aggregator.aggregate(model_params=model_params, weights=weights)
