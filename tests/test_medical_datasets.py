...
class TestMedicalFolderDataset(unittest.TestCase):
    ...
    def test_medical_folder_dataset_04_len(self):
        ...
        patcher = patch('fedbiomed.common.dataset._medical_datasets.MedicalFolderDataset.subject_folders',
                        return_value = [])
        patcher.start()
        dataset = MedicalFolderDataset(self.root)
        try:
    ...
    def test_medical_folder_dataset_08_demographics_getter(self):
        ...
        patcher = patch('fedbiomed.common.dataset._medical_datasets.MedicalFolderDataset.read_demographics',
                        side_effect=OSError)
        patcher.start()
        try:
            with self.assertRaises(FedbiomedDatasetError):
    ...
    def test_medical_folder_dataset_16_load_MedicalFolder(self):
        ...
        mfd_patcher = patch('fedbiomed.common.dataset.MedicalFolderDataset.__init__', side_effect=FedbiomedDatasetError)
        mfd_patcher.start()
        with self.assertRaises(FedbiomedDatasetError):
            medical_folder_controller.load_MedicalFolder()
        mfd_patcher.stop()
