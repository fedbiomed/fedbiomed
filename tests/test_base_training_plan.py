...
class TestBaseTrainingPlan(unittest.TestCase):
    ...
    def test_base_training_plan_09_n_iterations(self):
        tp = SimpleTrainingPlan()
        tp.training_data_loader = MagicMock()
        len_data_loader = 13
        tp.training_data_loader.__len__.return_value = len_data_loader

        # Researcher asks for 1 epoch
        epochs, remainder_batches, num_batches_per_epoch = tp._n_training_iterations({
            'epochs': 1,
            'batch_maxnum': None,
            'num_updates': None
        })
        ...
        self.assertEqual(epochs, 1)
        self.assertEqual(remainder_batches, 0)
        self.assertEqual(num_batches_per_epoch, 3)
        ...
        epochs, remainder_batches, num_batches_per_epoch = tp._n_training_iterations({
            'epochs': None,
            'batch_maxnum': None,
            'num_updates': len_data_loader - 3
        })
        self.assertEqual(epochs, 0)
        self.assertEqual(remainder_batches, len_data_loader - 3)
        self.assertEqual(num_batches_per_epoch, len_data_loader)
        ...
        epochs, remainder_batches, num_batches_per_epoch = tp._n_training_iterations({
            'epochs': None,
            'batch_maxnum': None,
            'num_updates': 2*len_data_loader + 3
        })
        self.assertEqual(epochs, 2)
        self.assertEqual(remainder_batches, 3)
        self.assertEqual(num_batches_per_epoch, len_data_loader)
