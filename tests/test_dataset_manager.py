...
class TestDatasetManager(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.fake_dataset = TestDatasetManager.FakePytorchDataset(
        ...
        self.tempdir = (
            tempfile.mkdtemp()
        )  # Creates and returns tempdir to save temp images
    ...
    def test_dataset_manager_02_get_by_id(self, tinydb_get_patch):
        """
    ...
    def test_dataset_manager_04_search_by_tags(
        self, tinydb_search_patch, queries_all_patch
    ):
        """
    ...
    def test_dataset_manager_05_read_csv_with_header(self):
        ...
        res = self.dataset_manager.read_csv(
            os.path.join(self.testdir, "csv", "tata-header.csv")
        )
    ...
    def test_dataset_manager_06_red_csv_without_header(self):
        ...
        # action
        res = self.dataset_manager.read_csv(
            os.path.join(self.testdir, "csv", "titi-normal.csv")
        )
        # checks if the file has been correctly parsed
        self.assertIsInstance(res, pd.DataFrame)
    ...
    def test_dataset_manager_09_load_default_database_as_dataset(
        self, dataset_mnist_patch
    ):
        """
        ...
        res_dataset = self.dataset_manager.load_default_database(
            database_name, database_path, as_dataset=True
        )
        # checks
        ...
        dataset_mnist_patch.assert_called_once_with(
            root=database_path, download=True, transform=mock.ANY
        )

    @patch("fedbiomed.node.dataset_manager.DatasetManager.get_torch_dataset_shape")
    @patch("torchvision.datasets.MNIST")
    def test_dataset_manager_10_load_default_database_as_dataset_false(
        self,
        dataset_mnist_patch,
        ...
        # action
        res_dataset = self.dataset_manager.load_default_database(
            database_name, database_path, as_dataset=False
        )
        # checks
    ...
    def test_dataset_manager_11_load_default_database_exception(self):
        ...
        with self.assertRaises(NotImplementedError):
            ...
            self.dataset_manager.load_default_database(
                "my_default_dataset", "/path/to/my/default/dataset"
            )

    @patch("torchvision.datasets.ImageFolder")
    def test_dataset_manager_12_load_images_dataset_as_dataset_true(
        self, imgfolder_patch
    ):
        """
        ...
        # action
        dataset = self.dataset_manager.load_images_dataset(
            database_path, as_dataset=True
        )

        # checks
        self.assertEqual(dataset, self.fake_dataset)
        imgfolder_patch.assert_called_once_with(database_path, transform=mock.ANY)
    ...
    def test_dataset_manager_13_load_images_dataset_as_dataset_false(self):
        ...
        """
        # arguments
        dataset_path = os.path.join(self.testdir, "images")

        # action
        res_dataset_shape = self.dataset_manager.load_images_dataset(
            dataset_path, as_dataset=False
        )
    ...
    def test_dataset_manager_14_load_csv_dataset(self, read_csv_patch):
        ...
        self.assertIsNone(np.testing.assert_array_equal(data.values, dummy_data.values))

    @patch("tinydb.table.Table.insert")
    @patch("fedbiomed.node.dataset_manager.DatasetManager.load_default_database")
    @patch("os.path.isdir")
    def test_dataset_manager_15_add_database_default_dataset(
        self,
        os_listdir_patch,
        ...
        # action
        dataset_id = self.dataset_manager.add_database(
        ...
        datasetmanager_load_default_dataset_patch.assert_called_once_with(
            fake_dataset_name, fake_dataset_path
        )
    ...
    def test_dataset_manager_16_add_database_real_csv_examples_based(self):
        ...
        dataset_id = self.dataset_manager.add_database(
            ...
            path=os.path.join(self.testdir, "csv", "tata-header.csv"),
            dataset_id=fake_dataset_id,
        )
        ...
        self.dataset_manager.add_database(
            ...
            data_type="csv",
            description="description",
            path=os.path.join(self.testdir, "csv", "titi-normal.csv"),
        )
        ...
        with self.assertRaises(Exception):
            self.dataset_manager.add_database(
                ...
                data_type="csv",
                description="description",
                path=os.path.join(self.testdir, "csv", "toto-error.csv"),
            )
        ...
        with self.assertRaises(AssertionError):
            dataset_id = self.dataset_manager.add_database(
                ...
                data_type="csv",
                description="description",
                path=os.path.join(self.testdir, "images"),
            )
    ...
    @patch("uuid.uuid4")
    def test_dataset_manager_18_add_database_real_images_example_based(
        self, uuid_patch
    ):
        """
    ...
    @patch("tinydb.table.Table.insert")
    @patch("uuid.uuid4")
    def test_dataset_manager_19_add_database_mednist(
        self, uuid_patch, insert_tinydb_patch
    ):
        # patchers:
        uuid_patch.return_value = FakeUuid()
        fake_dataset = self._create_fake_mednist_dataset()
        path = os.path.join(fake_dataset.root, os.pardir)
        datasetid = self.dataset_manager.add_database(
            name="test",
            tags=["mednist"],
            data_type="mednist",
            description="description",
        ...
        insert_tinydb_patch.assert_called_once_with(
            dict(
                name="test",
                data_type="mednist",
                tags=["mednist"],
                description="description",
                shape=[6, 3, 64, 64],
                path=os.path.join(fake_dataset.root, os.pardir, "MedNIST"),
    ...
    def test_dataset_manager_20_remove_database(self, get, db_remove_patch):
        """
    ...
    def test_dataset_manager_21_modify_database_info(
        self, tinydb_update_patch, conflicting_tags_patch
    ):
        """
        ...
        for new_dataset in new_dataset_list:
            ...
            tinydb_update_patch.assert_called_once_with(
                new_dataset, Query().dataset_id == dataset_id
            )
    ...
    @patch("fedbiomed.node.dataset_manager.DatasetManager.search_conflicting_tags")
    def test_dataset_manager_22_modify_database_info_error(
        self, conflicting_tags_patch
    ):
        """
        ...
        def conflicting_tags_side_effect(tags):
            return [
                {
                    "dataset_id": "one dataset is conflicting",
                    "name": "fake dataset with conflicting tags",
                }
            ]
    ...
    def test_dataset_manager_23_list_my_data(self, query_all_patch):
        """
    ...
    @patch("fedbiomed.node.dataset_manager.DatasetManager.load_default_database")
    def test_dataset_manager_24_load_as_dataloader_default(
        self, load_default_database_patch
    ):
        """
    ...
    def test_dataset_manager_25_load_as_dataloader_images(
        self, load_images_dataset_patch
    ):
        """
        Tests `load_as_dataloader` method where  the input
        ...
        # checks
        load_images_dataset_patch.assert_called_once_with(
            folder_path=fake_dataset["path"], as_dataset=True
        )
    ...
    def test_dataset_manager_26_load_existing_mednist_dataset(self):
        ...
        res_dataset_shape, path = self.dataset_manager.load_mednist_database(
            self.temp_dir.name, as_dataset=False
        )
        ...
        res_dataset, path = self.dataset_manager.load_mednist_database(
            self.temp_dir.name, as_dataset=True
        )
    ...
    def test_dataset_manager_27_download_extract_mednist(self, urlretrieve_patch):
        """
        ...
        def urlretieve_side_effect(url, path):
            ...
            os.rename(
                os.path.join(path, "MedNIST_test.tar.gz"),
        ...
        with patch.object(os, "remove") as os_remove_patch:
            ...
            # action
            res_dataset, pth = self.dataset_manager.load_mednist_database(
                self.temp_dir.name, as_dataset=True
            )
        # Tests
    ...
    def test_dataset_manager_28_load_mednist_database_exception(self):
        ...
        # 1. expected error:
        with patch(
            "fedbiomed.node.dataset_manager._dataset_manager.urlretrieve"
        ) as urlretrieve:
        ...
        # 2. unexpected error:
        with patch(
            "fedbiomed.node.dataset_manager._dataset_manager.datasets.ImageFolder"
        ) as image_folder_patch:
            image_folder_patch.side_effect = Exception(
    ...
    def test_dataset_manager_30_obfuscate_private_information(self):
        ...
        private_metadata = DatasetManager.obfuscate_private_information(
            metadata_with_private_info
        )
        expected_private_metadata = [
            {
                "nonprivate": "info",
                "data_type": "medical-folder",
                "dataset_parameters": {"nonprivate": "info"},
            }
        ]
        self.assertEqual(private_metadata, expected_private_metadata)
        with self.assertRaises(FedbiomedDatasetManagerError):
            _ = DatasetManager.obfuscate_private_information(
                [*metadata_with_private_info, "non-dict-like"]
            )
    ...
    def test_dataset_manager_31_data_loading_plan_save(self, patch_isdir):
        ...
        dlp_metadata, loading_blocks_metadata = dataset_manager.get_dlp_by_id(
            dataset["dlp_id"]
        )
        ...
        self.assertIn(
            LoadingBlockTypesForTesting.OTHER_LOADING_BLOCK_FOR_TESTING, new_dlp
        )
        self.assertDictEqual(
            new_dlp[LoadingBlockTypesForTesting.LOADING_BLOCK_FOR_TESTING].data,
            dlb1.data,
        )
        self.assertDictEqual(
            new_dlp[LoadingBlockTypesForTesting.OTHER_LOADING_BLOCK_FOR_TESTING].data,
            dlb2.data,
        )
