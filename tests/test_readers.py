...
def test_nifti_reader_read():
    ...
    with pytest.raises(FileNotFoundError):
        NiftiReader.read(nifti_file_path)
...
def test_nifti_reader_validate(monkeypatch):
    """Test the validation of a NIfTI file."""

    monkeypatch.setattr(Path, "exists", lambda self: False)

    with pytest.raises(FileNotFoundError):
        NiftiReader.validate(Path("path/to/nonexistent.nii"))

    monkeypatch.setattr(Path, "exists", lambda self: True)
    monkeypatch.setattr(Path, "is_file", lambda self: False)
    with pytest.raises(ValueError):
        NiftiReader.validate(Path("path/to/invalid_file.txt"))
    ...
    monkeypatch.setattr(Path, "is_file", lambda self: True)
    NiftiReader.validate(valid_path)
