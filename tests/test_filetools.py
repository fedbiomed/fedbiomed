...
class TestFiletools(unittest.TestCase):
    ...
    def test_filetools_01_create_exp_folder(self):
        ...
        self.assertEqual(
            exp_folder,
            filetools.create_exp_folder(config.vars["EXPERIMENTS_DIR"], exp_folder),
        )
        ...
        exp_folders = ["./toto/dir", "a/b", "/tmp/var/", self.testdir]
    ...
    def test_filetools_02_choose_bkpt_file(self):
        ...
        self.assertEqual(
            os.path.basename(bkpt_folder),
            breakpoint_folder_name + str("{:04d}".format(0)),
        )
        self.assertEqual(
            bkpt_file, breakpoint_folder_name + str("{:04d}".format(0)) + ".json"
        )
        ...
        self.assertEqual(
            bkpt_file, breakpoint_folder_name + str("{:04d}".format(2)) + ".json"
        )
        self.assertTrue(os.path.isdir(bkpt_folder))
    ...
    def test_filetools_04_create_unique_link(self):
        ...
        link_path = filetools.create_unique_link(
            self.testdir, link_prefix, link_suffix, "any_target_is_ok"
        )
        self.assertEqual(
            link_path, os.path.join(self.testdir, link_prefix + link_suffix)
        )
        ...
        for index in range(1, 4):
            ...
            self.assertEqual(
                ...
                os.path.join(
                    self.testdir,
                    link_prefix + "_" + str("{:02d}".format(index)) + link_suffix,
                ),
            )
            self.assertTrue(os.path.islink(link_path))
    ...
    def test_filetools_05_create_unique_file_link(self, patch_create_ul):
        """
        Test `create_unique_file_link` method
        """

        def side_create_ul(
            bkpt_folder_path, link_src_prefix, link_src_postfix, link_target
        ):
        ...
        for file, folder in zip(file_paths, bkpt_folder_paths):
            self.assertRaises(
                ValueError, filetools.create_unique_file_link, folder, file
            )
    ...
    def test_filetools_06_private_get_latest_file(self):
        ...
        # test 1
        files = ["Experiment_0", "Experiment_4", "Experiment_5", "blabla", "99_blabla"]

        pathfile_test = "/path/to/a/file"

        latest_file = filetools._get_latest_file(
            pathfile_test, files, only_folder=False
        )
        ...
        patcher_builtin_os_path_isdir = patch("os.path.isdir", return_value=True)
        patcher_builtin_os_path_isdir.start()
        latest_file = filetools._get_latest_file(pathfile_test, files, only_folder=True)
        self.assertEqual(files[2], latest_file)
    ...
    @patch("os.path.isdir")
    @patch("os.listdir")
    def test_filetools_08_private_find_breakpoint_path_2(
        self, patch_os_listdir, patch_os_path_isdir, patch_get_latest_file
    ):
        ...
        latest_bkpt_folder = os.path.join(
            config.vars["EXPERIMENTS_DIR"], "breakpoint", "breakpoint"
        )
    ...
    @patch("os.path.isdir")
    @patch("os.listdir")
    def test_filetools_09_private_find_breakpoint_path_raise_err1(
        self, patch_os_listdir, patch_os_path_isdir
    ):
    ...
    @patch("os.path.isdir")
    @patch("os.listdir")
    def test_filetools_10_private_find_breakpoint_path_raise_err2(
        self, patch_os_listdir, patch_os_path_isdir, patch_os_path_isfile
    ):
    ...
    @patch("os.path.isdir")
    @patch("os.listdir")
    def test_filetools_12_private_find_breakpoint_path_raise_err4(
        self, patch_os_listdir, patch_os_path_isdir
    ):
    ...
    @patch("os.path.isdir")
    @patch("os.listdir")
    def test_filetools_13_private_find_breakpoint_raise_err_5(
        self, patch_os_listdir, patch_os_path_isdir
    ):
        # test 7: test if error is raised when json file
        # not found in a breakpoint folder specified by user
        bkpt_folder = "/path/to/breakpoint"
        patch_os_listdir.return_value = ["one_file", "another_file"]
        patch_os_path_isdir.return_value = True
