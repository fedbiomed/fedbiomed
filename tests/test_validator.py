...
class TestValidator(unittest.TestCase):
    ...
    def test_validator_03_registration(self):
        ...
        self.assertTrue(
            v.register(rule_name, self.hook_01_positive_integer_check, override=True)
        )
    ...
    def test_validator_07_use_scheme(self):
        ...
        training_args_scheme = {
            "lr": {
                "rules": [float, self.loss_rate_validation_hook],
                "required": True,
                "default": 1.0,
            },
        }
        ...
        self.assertTrue(Validator().validate({"lr": 0.1}, training_args_scheme))

        with self.assertRaises(ValidateError):
            Validator().validate(
                {"lr": 0.2, "extra": "extra field"}, training_args_scheme
            )

        with self.assertRaises(ValidateError):
            Validator().validate({"lr": 2.2}, training_args_scheme)

        # same, but lr is not required
        training_args_scheme = {
            "lr": {"rules": [float, self.loss_rate_validation_hook], "default": 1.0},
        }

        self.assertTrue(Validator().validate({}, training_args_scheme))

        self.assertTrue(Validator().validate({"lr": 0.3}, training_args_scheme))

        with self.assertRaises(ValidateError):
            Validator().validate(
                {"lr": 0.4, "extra": "extra field"}, training_args_scheme
            )

        with self.assertRaises(ValidateError):
            Validator().validate({"lr": 1.4}, training_args_scheme)
        ...
        self.assertTrue(Validator().validate({"lr": 0.5}, training_args_scheme))

        with self.assertRaises(ValidateError):
            Validator().validate(
                {"lr": 0.6, "extra": "extra field"}, training_args_scheme
            )
    ...
    def test_validator_08_validate_the_validator(self):
        ...
        self.assertTrue(Validator().validate({"lr": 0.7}, "tr_01"))

        # or directly
        self.assertTrue(Validator().validate({"lr": 0.8}, training_args_ok))

        with self.assertRaises(ValidateError):
            Validator().validate({"lr": 2.71281}, training_args_ok)

        with self.assertRaises(ValidateError):
            Validator().validate({"lr": "toto"}, training_args_ok)

        training_args_ko = {
            "lr": {
                "rules": [self.loss_rate_validation_hook],
                "required": True,
                "unallowed_key": False,
            },
        }
    ...
    def test_validator_10_validator_hook_type(self):
        ...
        # invalid Type
        self.assertEqual(Validator()._hook_type(3.14), _ValidatorHookType.INVALID)

        self.assertEqual(Validator()._hook_type(None), _ValidatorHookType.INVALID)

        # builtin classes
        self.assertEqual(Validator()._hook_type(int), _ValidatorHookType.TYPECHECK)

        self.assertEqual(Validator()._hook_type(float), _ValidatorHookType.TYPECHECK)

        # non builtin classes
        self.assertEqual(
            Validator()._hook_type(Validator), _ValidatorHookType.TYPECHECK
        )
        ...
        self.assertEqual(Validator()._hook_type(v), _ValidatorHookType.SCHEME_VALIDATOR)

        self.assertEqual(
            Validator()._hook_type(SchemeValidator), _ValidatorHookType.TYPECHECK
        )

        self.assertEqual(
            Validator()._hook_type({}), _ValidatorHookType.SCHEME_AS_A_DICT
        )
...
class TestSchemeValidator(unittest.TestCase):
    ...
    def test_scheme_validator_01_validate_the_validator(self):
        ...
        training_args_scheme = {
            "loss": {"rules": [float], "required": True, "unallowed_key": False},
        }
        with self.assertRaises(RuleError):
            v = SchemeValidator(training_args_scheme)
    ...
    def test_scheme_validator_04_default_value_injection(self):
        ...
        sc = SchemeValidator(
            {
                "a": {"rules": [float], "required": True, "default": 3.14},
                "b": {"rules": [int], "required": True, "default": 666},
                "c": {"rules": [str], "default": "stupid because unused"},
                "d": {"rules": [str]},
            }
        )
        self.assertTrue(sc.is_valid())
        ...
        good_again = sc.populate_with_defaults(
            {"d": "some string"}, only_required=False
        )
