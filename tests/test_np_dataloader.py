...
class TestNPDataLoader(unittest.TestCase):
    ...
    def iterate_and_assert(self, batch_size, n_epochs, drop_last=False):
        dataloader = NPDataLoader(
            dataset=self.X, target=self.X, batch_size=batch_size, drop_last=drop_last
        )
        ...
        if remainder == 0:
            ...
            if not drop_last:
                ...
                self.assertNPArrayEqual(
                    outcome[-1], self.X[-batch_size - remainder : -remainder]
                )

    def test_npdataloader_00_creation(self):
        _ = NPDataLoader(
            dataset=self.X, target=self.X
        )  # Base case that should not raise errors

        with self.assertRaises(FedbiomedTypeError):
            _ = NPDataLoader(dataset="wrong-type", target=self.X)
        ...
        loader = NPDataLoader(dataset=np.squeeze(self.X), target=np.squeeze(self.X))
        self.assertIterableEqual(loader.dataset.shape, loader.target.shape)

        # test that wrong dataset shape raises ValueError
        with self.assertRaises(FedbiomedValueError):
            _ = NPDataLoader(dataset=self.X[:, np.newaxis], target=self.X)

        with self.assertRaises(FedbiomedValueError):
            _ = NPDataLoader(dataset=self.X, target=self.X, batch_size="wrong-type")

        with self.assertRaises(FedbiomedValueError):
            _ = NPDataLoader(dataset=self.X, target=self.X, batch_size=-1)

        with self.assertRaises(FedbiomedTypeError):
            _ = NPDataLoader(dataset=self.X, target=self.X, random_seed="wrong-type")

        # ensure that an unknown argument raises TypeError (this is used in SKLearnDataManager.split)
        with self.assertRaises(TypeError):
            _ = NPDataLoader(dataset=self.X, target=self.X, unknown_argument="unknown")
    ...
    def test_npdataloader_01_iteration(self):
        ...
        batch_size = 2
        dataloader = NPDataLoader(
            dataset=self.X, target=3.0 * self.X + 1.0, batch_size=batch_size
        )
        ...
        expected_target_sum = (3.0 * self.X + 1).sum()
    ...
    def test_npdataloader_02_shuffle(self):
        dataloader = NPDataLoader(
            dataset=self.X, target=self.X, batch_size=1, shuffle=True, random_seed=42
        )
        outcome = list()
        for data, target in dataloader:
            outcome.append(data)
    ...
    def test_npdataloader_03_target(self):
        dataloader = NPDataLoader(
            dataset=self.X, target=self.X, batch_size=1, random_seed=42
        )
        ...
        dataloader = NPDataLoader(
            dataset=self.X, target=self.X, batch_size=3, random_seed=42
        )
        for epoch in range(2):
            for data, target in dataloader:
        ...
        dataloader = NPDataLoader(
            dataset=self.X, target=self.X, batch_size=3, drop_last=True, random_seed=42
        )
        for epoch in range(2):
            for data, target in dataloader:
                self.assertNPArrayEqual(data, target)
