...
class TorchTrainingPlan(BaseTrainingPlan, nn.Module):
    ...
    def _set_device(self, use_gpu: Union[bool, None], node_args: dict):
        ...
        use_cuda = cuda_available and (
            (use_gpu and node_args["gpu"]) or node_args["gpu_only"]
        )

        if node_args["gpu_only"] and not cuda_available:
            logger.error(
                "Node wants to force model training on GPU, but no GPU is available"
            )
        if use_cuda and not use_gpu:
            logger.warning(
                "Node enforces model training on GPU, though it is not requested by researcher"
            )
        ...
        if use_cuda:
            if node_args["gpu_num"] is not None:
                if node_args["gpu_num"] in range(torch.cuda.device_count()):
                    ...
                    logger.warning(
                        f"Bad GPU number {node_args['gpu_num']}, using default GPU"
                    )
                    self.device = "cuda"
            else:
                self.device = "cuda"
        logger.debug(
            f"Using device {self.device} for training "
    ...
    def send_to_device(
        self, to_send: Union[torch.Tensor, list, tuple, dict], device: torch.device
    ):
        ...
        if isinstance(to_send, torch.Tensor):
            ...
            return {
                key: self.send_to_device(val, device) for key, val in to_send.items()
            }
            ...
            raise FedbiomedTrainingPlanError(
                f"{ErrorNumbers.FB310.value} cannot send data to device. "
    ...
    def training_routine(
        ...
        for epoch in range(1, epochs + 1):
            ...
            for batch_idx, (data, target) in enumerate(self.training_data_loader):
                ...
                data, target = self.send_to_device(
                    data, self.device
                ), self.send_to_device(target, self.device)
                ...
                if fedprox_mu is not None:
                    try:
                        ...
                        msg = (
                            ...
                            + ": fedprox_mu parameter requested is not a float"
                        )
                        logger.critical(msg)
                        raise FedbiomedTrainingPlanError(msg)
                ...
                if (batch_maxnum > 0) and (batch_ >= batch_maxnum):
                    ...
                    logger.info(
                        "Reached {} batches for this epoch, ignore remaining data".format(
                            batch_maxnum
                        )
                    )
                    break
        ...
        with torch.no_grad():
            ...
            for batch_ndx, (data, target) in enumerate(self.testing_data_loader):
                ...
                if hasattr(self, "testing_step"):
                    ...
                    if m_value is None:
                        msg = (
                            ErrorNumbers.FB605.value + ": metric function returned None"
                        )
                ...
                # Use accuracy as default metric
                    ...
                    try:
                        ...
                        msg = ErrorNumbers.FB605.value + ": error - " + str(e)
                        logger.critical(msg)
                        raise FedbiomedTrainingPlanError(msg)
                    ...
                    m_value = metric_controller.evaluate(
                        y_true=y_true, y_pred=predicted, metric=metric, **metric_args
                    )

                metric_dict = self._create_metric_result_dict(
                    m_value, metric_name=metric_name
                )

                logger.debug(
                    "Validation: Batch {} [{}/{}] | Metric[{}]: {}".format(
                        str(batch_),
                        batch_ * len(target),
                        tot_samples,
                        metric_name,
    ...
    def after_training_params(self, vector=False) -> Union[list, dict]:
        ...
        try:
            ...
            pass
        if vector:
            return torch.nn.utils.parameters_to_vector(self.parameters()).tolist()
    ...
    def __preprocess(self):
        ...
        for (name, process) in self.pre_processes.items():
            ...
            if process_type == ProcessTypes.DATA_LOADER:
                ...
                logger.error(
                    f"Process `{process_type}` is not implemented for `TorchTrainingPlan`. Preprocess will "
                    f"be ignored"
                )
    ...
    def __process_data_loader(self, method: Callable):
        ...
        if isinstance(data_loader, type(self.training_data_loader)):
            ...
            logger.debug(
                f"Data loader for training routine has been updated by the process `{method.__name__}` "
            )
        else:
            msg = (
                ErrorNumbers.FB605.value
