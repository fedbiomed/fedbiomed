...
class MedicalFolderBase:
    ...
    def is_modalities_existing(self, subject: str, modalities: List[str]) -> List[bool]:
        ...
        if not isinstance(modalities, list):
            raise FedbiomedDatasetError(
                f"Expected a list for modalities, but got {type(modalities)}"
            )
        return [
            self._root.joinpath(subject, modality).is_dir() for modality in modalities
        ]
    ...
    def validate_MedicalFolder_root_folder(path: Union[str, Path]) -> Path:
        ...
        if not path.is_dir():
            raise FedbiomedDatasetError(
                "Root for Medical Folder dataset should be a directory."
            )
...
class MedicalFolderDataset(Dataset, MedicalFolderBase):
    ...
    def get_nontransformed_item(self, item):
        ...
        if not subjects:
            ...
            raise FedbiomedDatasetError(
                f"{ErrorNumbers.FB613.value}: Cannot find subject folders"
            )
        # Get subject folder
    ...
    def set_dataset_parameters(self, parameters: dict):
        ...
        if not isinstance(parameters, dict):
            raise FedbiomedDatasetError(
                f"Expected type for `parameters` is `dict, but got {type(parameters)}`"
            )
        ...
        for key, value in parameters.items():
            if hasattr(self, key):
                ...
                logger.warning(
                    f"Trying to set undefined attribute {key} ti MedicalFolderDataset"
                )
...
class MedicalFolderController(MedicalFolderBase):
    ...
    def check_modalities(self, _raise: bool = True) -> Tuple[bool, str]:
        ...
        if len(unique_modalities) == len(modalities):
            message = (
                f"{ErrorNumbers.FB613.value}: Subject folders in Medical Folder root folder does not contain"
