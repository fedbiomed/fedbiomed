...
class Environ(metaclass=SingletonMeta):
    ...
    def __getitem__(self, key: str):
        ...
        if key not in self._values:
            _msg = (
                ErrorNumbers.FB600.value
                + ": config file does not contain the key: "
                + str(key)
            )
            logger.critical(_msg)
            raise FedbiomedEnvironError(_msg)
    ...
    def __setitem__(self, key: str, value: Any):
        ...
        if value is None:
            _msg = (
                ErrorNumbers.FB600.value
                + ": cannot set value to None for key: "
                + str(key)
            )
            logger.critical(_msg)
            raise FedbiomedEnvironError(_msg)
        ...
        for _key in "CONFIG_DIR", "VAR_DIR", "CACHE_DIR", "TMP_DIR":
            ...
            if not os.path.isdir(dir):
                try:
                    ...
                    _msg = (
                        ErrorNumbers.FB600.value
                        + ": cannot create environment subtree in: "
                        + dir
                    )
                    logger.critical(_msg)
                    raise FedbiomedEnvironError(_msg)
    ...
    def _init_researcher(self):
        ...
        self._values["RESEARCHER_ID"] = os.getenv("RESEARCHER_ID", _cfg_value)
    ...
    def _init_node(self):
        ...
        try:
            ...
            _msg = (
                ErrorNumbers.FB600.value
                + ": no default/node_id in config file, please recreate a new config file"
            )
            logger.critical(_msg)
            raise FedbiomedEnvironError(_msg)
        ...
        self._values["MESSAGES_QUEUE_DIR"] = os.path.join(
            VAR_DIR, f"queue_manager_{NODE_ID}"
        )
        self._values["DB_PATH"] = os.path.join(VAR_DIR, f"db_{NODE_ID}.json")

        self._values["DEFAULT_MODELS_DIR"] = os.path.join(
            ROOT_DIR, "envs", "common", "default_models"
        )
        ...
        self._values["ALLOW_DEFAULT_MODELS"] = os.getenv(
            "ALLOW_DEFAULT_MODELS", _cfg_value
        ...
        self._values["MODEL_APPROVAL"] = os.getenv(
            "ENABLE_MODEL_APPROVAL", _cfg_value
        ).lower() in ("true", "1", "t", True)
        ...
        if os.getenv("CONFIG_FILE"):
            ...
            if not os.path.isabs(CONFIG_FILE):
                CONFIG_FILE = os.path.join(
                    self._values["CONFIG_DIR"], os.getenv("CONFIG_FILE")
                )
        else:
            if self._values["COMPONENT_TYPE"] == ComponentType.RESEARCHER:
                CONFIG_FILE = os.path.join(
                    self._values["CONFIG_DIR"], "config_researcher.ini"
                )
            else:
                CONFIG_FILE = os.path.join(
                    self._values["CONFIG_DIR"], "config_node.ini"
                )
    ...
    def _create_node_config_file(self, cfg: dict, config_file: str):
        ...
        cfg["default"] = {"node_id": node_id, "uploads_url": uploads_url}
        ...
        try:
            ...
            _msg = (
                ErrorNumbers.FB600.value + ": cannot save config file: " + config_file
            )
            logger.critical(_msg)
            raise FedbiomedEnvironError(_msg)
    ...
    def _create_researcher_config_file(self, cfg: dict, config_file: str):
        ...
        cfg["default"] = {"researcher_id": researcher_id, "uploads_url": uploads_url}
    ...
    def _init_network_configurations(self, cfg: dict):
        ...
        try:
            ...
            _msg = (
                ErrorNumbers.FB600.value
                + ": no mqtt/broker_ip in config file, please recreate a new config file"
            )
            logger.critical(_msg)
            raise FedbiomedEnvironError(_msg)

        self._values["MQTT_BROKER"] = os.getenv("MQTT_BROKER", _cfg_value)
        ...
        try:
            ...
            _msg = (
                ErrorNumbers.FB600.value
                + ": no mqtt/port in config file, please recreate a new config file"
            )
            logger.critical(_msg)
            raise FedbiomedEnvironError(_msg)

        self._values["MQTT_BROKER_PORT"] = int(
            os.getenv("MQTT_BROKER_PORT", _cfg_value)
        )
    ...
    def info(self):
        ...
        if self._values["COMPONENT_TYPE"] == ComponentType.NODE:
            ...
            logger.info(
                "allow_default_model = " + str(self._values["ALLOW_DEFAULT_MODELS"])
            )
