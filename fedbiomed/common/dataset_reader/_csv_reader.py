...
class CsvReader:
    ...
    def normalize_columns(self, columns: Iterable | int | str) -> list[str]:
        ...
        if not isinstance(columns, Iterable):
            raise FedbiomedUserInputError(
                "Invalid type for columns: expected str, int, or iterable of int/str, "
                f"got {type(columns)}"
            )
        if not all(isinstance(item, (str, int)) for item in columns):
            raise FedbiomedUserInputError(
                "Invalid type for columns: expected all items to be str or int"
            )

        # Check for invalid column indexes
        n_cols = len(self.columns)
        _inter = list(
            filter(lambda x: isinstance(x, int) and (0 > x or x > n_cols), columns)
        )
        if any(_inter):
            raise FedbiomedUserInputError(
                f"Column index(es) {_inter} out of range (0 to {n_cols - 1})"
            )

        columns = list(columns)  # make a copy to avoid modifying input
        # Convert integer indexes to column names
        columns = [self.columns[i] if isinstance(i, int) else i for i in columns]
        ...
        # Warn if duplicate columns are requested
        _duplicates = [item for item in set(columns) if columns.count(item) > 1]
        if any(_duplicates):
            raise FedbiomedUserInputError(
                f"Duplicate columns {_duplicates} found in the requested columns"
            )
