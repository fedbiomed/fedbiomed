...
class CsvReader(Reader):
    ...
    def __init__(
        ...
        if self._memory:
            ...
            self._shape = self._reader.collect().shape

        self._len = self._shape[0]

    # Nota: does not include filtering of DLP, which is unknown to Reader
    def read(self, **kwargs) -> Union[pl.DataFrame, pl.LazyFrame]:
        """
    ...
    def _cpu_reader(self, **reader_kwargs):
        """
        Reads the CSV file using Polars in memory.
        If `memory` is set to False, it will read the file lazily.
        Args:
