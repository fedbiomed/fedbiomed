...
class SGDSkLearnModel():
    ...
    def set_init_params(self, init_params):
        for p in init_params.keys():
            ...
            setattr(self.reg, p, init_params[p])
    ...
    def after_training_params(self):
        return {key: getattr(self.reg, key) for key in self.param_list}
        ...
        if params is None:
            ...
            if params.get('model_params') is not None: # called in the Round
                for p in params['model_params'].keys():
                    setattr(self.reg, p, params['model_params'][p])
            else:
                for p in params.keys():
                    setattr(self.reg, p, params[p])
            dump(self.reg, file)
        file.close()
    ...
    def load(self, filename, to_params: bool = False):
        ...
        if not to_params:
            ...
            self.reg =  load(file)
            di_ret['model_params'] = {key: getattr(self.reg, key)  for key in self.param_list} 
        file.close()
        return di_ret
