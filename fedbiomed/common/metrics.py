...
class Metrics(object):
    ...
    def _configure_y_true_pred_(
        ...
        if len(y_pred) != len(y_true):
            raise FedbiomedMetricError(
                f"{ErrorNumbers.FB611.value}: Predictions or true values should have"
        ...
        if metric.metric_category() is _MetricCategory.CLASSIFICATION_LABELS:
            ...
            if output_shape_y_pred != output_shape_y_true:
                raise FedbiomedMetricError(
                    f"{ErrorNumbers.FB611.value}: For the metric `{metric.name}` multiple "
    ...
    def _configure_multiclass_parameters(
        ...
        if len(np.unique(y_true)) > 2:
            ...
            logger.info(
                f"Actual/True values (y_true) has more than two levels, using multiclass `{average}` "
