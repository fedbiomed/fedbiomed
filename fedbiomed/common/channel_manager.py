...
class ChannelManager:
    ...
    def list(self) -> List[str]:
        ...
        try:
            ...
            errmess = (
                f'{ErrorNumbers.FB631.value}: failed listing elements in the database table "{self._table}" '
                f"with error: {e}"
            )
            raise FedbiomedNodeToNodeError(errmess) from e
    ...
    def get(self, distant_node_id: str) -> Union[dict, None]:
        ...
        try:
            ...
            errmess = (
                f'{ErrorNumbers.FB631.value}: failed searching the database table "{self._table}" '
    ...
    def add(self, distant_node_id: str, local_key: bytes):
        ...
        try:
            ...
            errmess = (
                f'{ErrorNumbers.FB631.value}: failed adding an entry in table "{self._table}" '
