...
class FederatedAnalytics:
    ...
    def basic_stats(
        ...
        """Returns analytics result wrapper for basic statistics (min, max, count, mean, std)."""
        dataset_type = self._validate_if_dataset_has_analytics(
            AnalyticsTypes.BASIC_STATS.value
        )
        self._validate_dataset_arguments(dataset_args)
        node_replies, errors = self._compute_analytics(
            AnalyticsTypes.BASIC_STATS.value, dataset_args, fa_args
        )
        return AnalyticsResult(
            dataset_type=dataset_type,
            analytics_type=AnalyticsTypes.BASIC_STATS,
            node_results=node_replies,
            errors=errors,
        )
    ...
    def histogram(
        ...
        Returns:
            AnalyticsResult wrapper that provides aggregated and per-node histogram results.
        """
        if self._fds is None:
            raise FedbiomedError(
                "No defined FederatedDataset found for FederatedAnalytics."
        ...
        return AnalyticsResult(
            dataset_type=dataset_type,
            analytics_type=AnalyticsTypes.HISTOGRAM,
            node_results=node_histograms,
            errors=errors,
        )
