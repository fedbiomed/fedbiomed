...
TrainingPlanT = TypeVar(
    "TrainingPlanT", Type[TorchTrainingPlan], Type[SKLearnTrainingPlan]
)
TrainingPlanWorkflowT = TypeVar(
    "TrainingPlanWorkflowT", bound="TrainingPlanWorkflow"
)  # only for typing
T = TypeVar("T")
...
class TrainingPlanWorkflow(FederatedWorkflow, ABC):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if training_plan_class is not None and not issubclass(
            training_plan_class, TRAINING_PLAN_TYPES
        ):
            raise FedbiomedTypeError(
        ...
        self._training_args: Optional[TrainingArgs] = (
            None  # FIXME: is it ok to have this here?
        )
    ...
    def _instantiate_training_plan(self) -> Tuple[BaseTrainingPlan, str]:
        ...
        training_plan_file = os.path.join(
            self.experimentation_path(), training_plan_module + ".py"
        )
    ...
    @exp_exceptions
    def _update_training_plan(self, keep_weights: bool = True) -> None:
        ...
        if self._training_plan_class is None:
            ...
            with self._keep_weights(keep_weights):
                self._training_plan, self._training_plan_file = (
                    self._instantiate_training_plan()
                )
    ...
    @exp_exceptions
    def info(
        self, info: Optional[Dict] = None, missing: str = ""
        ...
        info["Arguments"].extend(
            ["Training Plan Class", "Model Arguments", "Training Arguments"]
        )
        info["Values"].extend(
            [
    ...
    def set_training_args(
        self, training_args: Union[dict, TrainingArgs, None]
    ) -> Union[dict, None]:
        ...
        if isinstance(training_args, TrainingArgs):
            ...
            msg = (
                f"{ErrorNumbers.FB410.value} in function `set_training_args`. "
    ...
    @exp_exceptions
    def set_training_plan_class(
        self, training_plan_class: Union[TrainingPlanT, None], keep_weights: bool = True
        ...
        if training_plan_class is None:
            ...
            if issubclass(training_plan_class, TRAINING_PLAN_TYPES):
                ...
                msg = (
                    ErrorNumbers.FB410.value
                    + f" `training_plan_class` : {training_plan_class} class"
                )
                logger.critical(msg)
                raise FedbiomedExperimentError(msg)
        else:
            ...
            msg = (
                ErrorNumbers.FB410.value
                + f" `training_plan_class` of type: {type(training_plan_class)}"
            )
            logger.critical(msg)
            raise FedbiomedExperimentError(msg)
    ...
    @exp_exceptions
    def set_model_args(self, model_args: dict, keep_weights: bool = True) -> dict:
    ...
    def check_training_plan_status(self) -> Dict:
        ...
        if self.training_data() is None:
            msg = (
                f"{ErrorNumbers.FB410.value}. Cannot check training plan status: training data is not defined."
    ...
    @exp_exceptions
    def breakpoint(self, state, bkpt_number) -> None:
        """
        ...
        state["training_plan_path"] = create_unique_link(
            ...
            "model_" + str("{:04d}".format(bkpt_number - 1)),
            ".py",
    ...
    def load_breakpoint(
        ...
        loaded_exp.set_training_args(
            TrainingArgs.load_state_breakpoint(saved_state.get("training_args"))
        )
    ...
    def _check_round_value_consistency(
        ...
        if not isinstance(round_current, int):
            msg = (
                ErrorNumbers.FB410.value
                + f" `{variable_name}` of type : {type(round_current)}"
            )
            logger.critical(msg)
            raise FedbiomedExperimentError(msg)

        if round_current < 0.0:
            # cannot set a round <0
            msg = (
                ErrorNumbers.FB410.value
                + f" `{variable_name}` cannot be negative or zero: {round_current}"
            )
            logger.critical(msg)
            raise FedbiomedExperimentError(msg)
