...
class Aggregator:
    ...
    def secure_aggregation(
        ...
        if len(validation) != 1 or not math.isclose(
            validation[0], secagg_random, abs_tol=0.01
        ):
            raise FedbiomedAggregatorError(
                "Aggregation is failed due to incorrect decryption."
            )
    ...
    def save_state_breakpoint(
        ...
        **aggregator_args_create: Any,
        ...
        if breakpoint_path:
            filename = self._save_arg_to_file(
                breakpoint_path, "aggregator_args", uuid.uuid4(), self._aggregator_args
            )
    ...
    def _save_arg_to_file(
        self, breakpoint_path: str, arg_name: str, node_id: str, arg: Any
    ) -> str:
        filename = os.path.join(
            breakpoint_path, f"{arg_name}_{str(node_id)[0:11]}_{uuid.uuid4()}.mpk"
        )
