...
class Overlay(metaclass=SingletonMeta):
    ...
    def __init__(self, grpc_client: GrpcController, pending_requests: EventWaitExchange):
        ...
        # Default keys
        default_n2n_key = self._load_default_n2n_key()
        self._local_node_private_key = default_n2n_key
        self._distant_node_public_key = default_n2n_key.public_key()
    ...
    @staticmethod
    def _load_default_n2n_key() -> rsa.RSAPrivateKey:
    ...
    def format_outgoing_overlay(self, message: Message) -> List[bytes]:
        ...
        if _CHUNK_SIZE * 8 > min(self._local_node_private_key.key_size, self._distant_node_public_key.key_size):
            raise FedbiomedNodeToNodeError(
        ...
        signed = Serializer.dumps({
            ...
            'signature': self._local_node_private_key.sign(
        ...
        return [
            self._distant_node_public_key.encrypt(
    ...
    def format_incoming_overlay(self, payload: List[bytes]) -> InnerMessage:
        ...
        try:
            decrypted_chunks = [
                self._local_node_private_key.decrypt(
                    chunk,
                    padding.OAEP(
                        mgf=padding.MGF1(algorithm=hashes.SHA256()),
                        algorithm=hashes.SHA256(),
        ...
        try:
            self._distant_node_public_key.verify(
                decrypted['signature'],
