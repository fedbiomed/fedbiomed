...
class Node:
    ...
    def on_message(self, msg: dict, topic: str = None):
        ...
        try:
            ...
            resid = (
                "researcher_id" in msg.keys()
                and msg["researcher_id"]
                or "unknown_researcher_id"
            )
            self.send_error(
                ErrorNumbers.FB301,
    ...
    def parser_task(self, msg: Union[bytes, str, Dict[str, Any]]):
        ...
        ), "classname for the model and training routine was not found in message."

        assert isinstance(
            model_class, str
        ), "`model_class` must be a string corresponding to the classname for the model and training routine in the repository"  # noqa
        ...
        if environ["NODE_ID"] in msg.get_param("training_data"):
            for dataset_id in msg.get_param("training_data")[environ["NODE_ID"]]:
                ...
                if data is None or "path" not in data.keys():
                    ...
                    logger.error(
                        "Did not found proper data in local datasets "
    ...
    def send_error(
        ...
        self.messaging.send_error(
            errnum, extra_msg=extra_msg, researcher_id=researcher_id
        )
