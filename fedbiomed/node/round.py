...
class Round:
    ...
    def run_model_training(self) -> dict[str, Any]:
        ...
        try:
            ...
            status, _ = self.repository.download_file(
                self.model_url, import_module + ".py"
            )
            ...
            if status != 200:
                ...
                if environ["MODEL_APPROVAL"]:
                    approved, model = self.model_manager.check_model_status(
                    ...
                    if not approved:
                        ...
                        logger.info(
                            f'Model has been approved by the node {model["name"]}'
                        )
        ...
        for arg in node_side_args:
            if arg in self.training_kwargs:
                ...
                logger.warning(
                    f"Researcher trying to set node-side training parameter {arg}. "
        ...
        training_kwargs_with_history = dict(
    ...
    def _set_training_testing_data_loaders(self):
        ...
        if (
            ...
            logger.warning(
                "Validation will not be perform for the round, since there is no validation activated. "
                "Please set `test_on_global_updates`, `test_on_local_updates`, or both in the "
                "experiment."
            )

        if test_ratio == 0 and (
            test_local_updates is False or test_global_updates is False
        ):
            logger.warning(
    ...
    def _split_train_and_test_data(self, test_ratio: float = 0):
        ...
        if len(args) > 1 and "batch_size" not in args:
            raise FedbiomedRoundError(
                f"{ErrorNumbers.FB314.value}, `the arguments of `training_data` of training "
        ...
        try:
            ...
            raise FedbiomedRoundError(
                f"{ErrorNumbers.FB314.value}: Error while loading data manager; {str(e)}"
            )
