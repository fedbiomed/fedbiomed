...
modelsSearchScheme = SchemeValidator(
    {
        "by": {"rules": [str], "required": True},
...
class ModelManager:
    ...
    def __init__(self):
        ...
        self._tags_to_remove = ["model_path", "hash", "date_modified", "date_created"]
    ...
    def _create_hash(path: str):
        ...
        if not isinstance(path, str):
            raise FedbiomedModelManagerError(
                ErrorNumbers.FB606.value + f": {path} is not a path"
            )
        ...
        try:
            ...
            raise FedbiomedModelManagerError(
                ErrorNumbers.FB606.value + f": model file {path} not found on system"
            )
        except PermissionError:
            raise FedbiomedModelManagerError(
                ErrorNumbers.FB606.value
                + f": cannot open model file {path} due"
        ...
        try:
            ...
            raise FedbiomedModelManagerError(
                ErrorNumbers.FB606.value + f": cannot minify file {path}"
                f"details: {err}"
            )
        ...
        if hash_algo in HashingAlgorithms.list():
            ...
            raise FedbiomedModelManagerError(
                ErrorNumbers.FB606.value
                + ": unknown hashing algorithm in the `environ`"
    ...
    def _check_model_not_existing(
        ...
        if name is not None:
            try:
                ...
                error = (
                    ErrorNumbers.FB606.value
                    + ": search request on database failed."
                    + f" Details: {str(err)}"
                )
        ...
        if path is not None:
            ...
            if models_path_get:
                error = (
                    ErrorNumbers.FB606.value
                    + f": this model has been added already: {path}"
                )
                logger.critical(error)
                raise FedbiomedModelManagerError(error)
    ...
    def check_hashes_for_registered_models(self):
        ...
        self._db.clear_cache()
        try:
            models = self._db.search(
                self._database.model_type.all(ModelTypes.REGISTERED.value)
            )
            ...
            for model in models:
                ...
                if os.path.isfile(model["model_path"]):
                    if model["algorithm"] != environ["HASHING_ALGORITHM"]:
                        logger.info(
                            f'Recreating hashing for : {model["name"]} \t {model["model_id"]}'
                        )
                    ...
                    logger.info(
                        f'Model : {model["name"]} could not found in : {model["model_path"]}, will be removed'
                    )
                    try:
    ...
    def check_model_status(
        self, model_path: str, state: Union[ModelApprovalStatus, ModelTypes, None]
        ...
        try:
            if _all_models_with_status is None:
                ...
                model = self._db.get(
                    _all_models_with_status & _all_models_which_have_req_hash
                )
        except Exception as e:
            raise FedbiomedModelManagerError(
                ErrorNumbers.FB606.value
                + f"database remove operation failed, with following error: {str(e)}"
            )
    ...
    def get_model_by_name(self, model_name: str) -> Union[Dict[str, Any], None]:
        ...
        if not isinstance(model_name, str):
            raise FedbiomedModelManagerError(
                ErrorNumbers.FB606.value + f": model name {model_name} is not a string"
            )
        ...
        try:
            ...
            raise FedbiomedModelManagerError(
                ErrorNumbers.FB606.value + ": cannot search database for model "
    ...
    def get_model_from_database(self, model_path: str) -> Union[Dict[str, Any], None]:
        ...
        if not isinstance(model_path, str):
            raise FedbiomedModelManagerError(
                ErrorNumbers.FB606.value + " : no model_path specified"
            )
        req_model_hash, _ = self._create_hash(model_path)
        ...
        try:
            ...
            raise FedbiomedModelManagerError(
                ErrorNumbers.FB606.value
                + f"database get operation failed, with following error: {str(e)}"
            )
    ...
    def get_model_by_id(
        self, model_id: str, secure: bool = True, content: bool = False
        ...
        if not isinstance(model_id, str):
            raise FedbiomedModelManagerError(
                ErrorNumbers.FB606.value + f": model_id {model_id} is not a string"
            )
    ...
    def reply_model_approval_request(self, msg: dict, messaging: Messaging):
        ...
        try:
            ...
            status, tmp_file = self._repo.download_file(
                msg["model_url"], model_name + ".py"
            )
            ...
            logger.error(
                f"Can not check whether model has already be registered or not due to error: {fed_err}"
            )
            downloadable_checkable = False
        ...
        if not is_existant and downloadable_checkable:
            ...
            try:
                ...
                # Model file creation date
                ctime = datetime.fromtimestamp(os.path.getctime(model_path)).strftime(
                    "%d-%m-%Y %H:%M:%S.%f"
                )
            except (PermissionError, FileNotFoundError, OSError) as err:
                reply["success"] = False
                logger.error(
                    f"Cannot save model '{msg['description']} 'into directory due to error : {err}"
                )
            else:
                try:
                    ...
                    reply["success"] = False
                    logger.error(
                        f"Cannot add model '{msg['description']} 'into database due to error : {err}"
                    )
                else:
                    reply["success"] = True
                    logger.debug(
                        f"Model '{msg['description']}' successfully received by Node for approval"
                    )

        elif is_existant and downloadable_checkable:
            if self.check_model_status(model_to_check, ModelApprovalStatus.PENDING)[0]:
                logger.info(
                    f"Model '{msg['description']}' already sent for Approval (status Pending). "
                    "Please wait for Node approval."
                )
            elif self.check_model_status(model_to_check, ModelApprovalStatus.APPROVED)[
                0
            ]:
                logger.info(
                    f"Model '{msg['description']}' is already Approved. Ready to train on this model."
                )
            else:
                logger.warning(
                    f"Model '{msg['description']}' already exists in database. Aborting"
                )
            reply["success"] = True
        else:
    ...
    def reply_model_status_request(self, msg: dict, messaging: Messaging):
        ...
        try:
            ...
            status, model_file = self._repo.download_file(
                msg["model_url"], model_name + ".py"
            )
            if status != 200:
            ...
            reply = {
                **header,
                ...
                "approval_obligation": False,
                "status": "Error",
                "msg": ErrorNumbers.FB604.value
                + ": An error occured when downloading model file."
            ...
            reply = {
                **header,
                ...
                "status": "Error",
                "msg": ErrorNumbers.FB606.value
                + ": An unknown error occured when downloading model file."
                f' {msg["model_url"]} , {e}',
            }
        # finally:
    ...
    def register_update_default_models(self):
        ...
        for model_name in models_deleted:
            try:
                ...
                logger.info(
                    "Removed default model file has been detected,"
        ...
        for model in models_exists:
            ...
            try:
                hash, algorithm = self._create_hash(
                    os.path.join(environ["DEFAULT_MODELS_DIR"], model)
                )
                ...
                if model_info["algorithm"] != environ["HASHING_ALGORITHM"]:
                    ...
                    logger.info(
                        f'Recreating hashing for : {model_info["name"]} \t {model_info["model_id"]}'
                    )
                    self._db.update(
                        {
                            "hash": hash,
                ...
                # If default model file is modified update hashing
                    ...
                    model_info["date_modified"], "%d-%m-%Y %H:%M:%S.%f"
                ):
                    ...
                    logger.info(
                        f"Modified default model file has been detected. Hashing will be updated for: {model}"
                    )
                    self._db.update(
                        {
                            "hash": hash,
    ...
    def _update_model_status(
        ...
        if not isinstance(model_status, ModelApprovalStatus):
            raise FedbiomedModelManagerError(
                ErrorNumbers.FB606.value
                + ": parameter model_status (ModelApprovalStatus) has bad "
                f"type {type(model_status)}"
            )
        if notes is not None and not isinstance(notes, str):
            raise FedbiomedModelManagerError(
                ErrorNumbers.FB606.value
                + ": parameter note (Union[str, None]) has bad "
                f"type {type(notes)}"
            )
        ...
        if model.get("model_status") == model_status.value:
            logger.warning(
                f" model {model_id} has already the following model status {model_status.value}"
            )
            return True
            ...
            try:
                ...
                raise FedbiomedModelManagerError(
                    ErrorNumbers.FB606.value
                    + f"database update operation failed, with following error: {str(e)}"
                )
    ...
    def approve_model(
        self, model_id: str, extra_notes: Union[str, None] = None
    ) -> True:
        ...
        Returns:
            Currently always returns True
        """
        res = self._update_model_status(
            model_id, ModelApprovalStatus.APPROVED, extra_notes
        )
        return res
    ...
    def reject_model(self, model_id: str, extra_notes: Union[str, None] = None) -> True:
        ...
        """
        res = self._update_model_status(
            model_id, ModelApprovalStatus.REJECTED, extra_notes
        )
        return res
    ...
    def delete_model(self, model_id: str) -> True:
        ...
        try:
            ...
            raise FedbiomedModelManagerError(
                ErrorNumbers.FB606.value + ": cannot get model from database."
                f"Details: {str(err)}"
            )

        if model is None:
            raise FedbiomedModelManagerError(
                ErrorNumbers.FB606.value + f": model {model_id} not in database"
            )
        ...
        if model["model_type"] != ModelTypes.DEFAULT.value:
            try:
                ...
                raise FedbiomedModelManagerError(
                    ErrorNumbers.FB606.value + ": cannot remove model from database."
                    f"Details: {str(err)}"
                )
        else:
            raise FedbiomedModelManagerError(
                ErrorNumbers.FB606.value + "For default models, please remove"
    ...
    def list_models(
        ...
        select_status: Union[
            None, ModelApprovalStatus, List[ModelApprovalStatus]
        ] = None,
        verbose: bool = True,
        ...
        if sort_by is not None and not isinstance(sort_by, str):
            raise FedbiomedModelManagerError(
                ErrorNumbers.FB606.value
                + f": parameter sort_by has bad type {type(sort_by)}"
            )
        if not isinstance(verbose, bool):
            raise FedbiomedModelManagerError(
                ErrorNumbers.FB606.value
                + f": parameter verbose has bad type {type(verbose)}"
            )
        # in case select_status is a list, we filter later with elements are ModelApprovalStatus
        if (
            select_status is not None
            and not isinstance(select_status, ModelApprovalStatus)
            and not isinstance(select_status, list)
        ):
            raise FedbiomedModelManagerError(
                ErrorNumbers.FB606.value
                + f": parameter select_status has bad type {type(select_status)}"
            )
        if search is not None and not isinstance(search, dict):
            raise FedbiomedModelManagerError(
                f"{ErrorNumbers.FB606.value}: `search` argument should be dictionary that "
        ...
        if isinstance(select_status, (ModelApprovalStatus, list)):
            ...
            select_status = [
                x.value for x in select_status if isinstance(x, ModelApprovalStatus)
            ]
            # extract value from ModelApprovalStatus
            try:
                if search:
                    models = self._db.search(
                        self._database.model_status.one_of(select_status)
                        & self._database[search["by"]].matches(
            ...
            try:
                ...
                raise FedbiomedModelManagerError(
                    ErrorNumbers.FB606.value
                    + f"database full read operation failed, with following error: {str(e)}"
                )
    ...
    def _remove_sensible_keys_from_request(self, doc: Dict[str, Any]):
        ...
        for tag_to_remove in self._tags_to_remove:
            try:
                ...
                logger.warning(
                    f"missing entry in database: {tag_to_remove} for model {doc}"
                )
