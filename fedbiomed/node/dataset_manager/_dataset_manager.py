...
class DatasetManager:
    ...
    def get_data_loading_blocks_by_ids(
        self, dlb_ids: Union[str, List[str]]
    ) -> List[dict]:
    ...
    def load_default_database(
        ...
        if "mnist" in name.lower():
            ...
            raise NotImplementedError(
                f"Default dataset `{name}` hasnot been implemented."
            )
        if as_dataset:
            return dataset
        else:
    ...
    ) -> Tuple[Union[List[int], torch.utils.data.Dataset], str]:
    ...
    def add_database(
        ...
        if data_type not in data_types:
            ...
            if (
                data_loading_plan is None
                or FlambyLoadingBlockTypes.FLAMBY_DATASET_METADATA
                not in data_loading_plan
            ):
                msg = (
                    f"{ErrorNumbers.FB316.value}. A DataLoadingPlan containing "
            ...
            try:
                ...
                dataset.set_dlp(
                    data_loading_plan
                )  # initializes fed_class as a side effect
    ...
    def remove_dlp_by_id(self, dlp_id: str):
        ...
        if not isinstance(dlp_id, str):
            _msg = (
                ErrorNumbers.FB316.value
                + f": Bad type for dlp '{type(dlp_id)}', expecting str"
            )
            logger.error(_msg)
            raise FedbiomedDatasetManagerError(_msg)
        if not str:
            _msg = (
                ErrorNumbers.FB316.value
                + ": Bad value for dlp, expecting non empty str"
            )
            logger.error(_msg)
            raise FedbiomedDatasetManagerError(_msg)
    ...
    def remove_database(self, dataset_id: str):
        ...
        _, dataset_document = self._dataset_table.get(
            self._database.dataset_id == dataset_id, add_docs=True
        )
    ...
    def modify_database_info(self, dataset_id: str, modified_dataset: dict):
        ...
        self._dataset_table.update(
            modified_dataset, self._database.dataset_id == dataset_id
        )
