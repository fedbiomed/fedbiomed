...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="MEDNIST sampler. Creates configuration files with sub-sets of the MedNIST dataset"
    )
    parser.add_argument(
        "-f",
        "--root_folder",
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="random seed to make MedNIST dataset splitting reproducible",
    )
    return parser.parse_args()


def manage_config_file(mednist_center_name: str = "mednist", **kwargs):
...
def ask_nb_sample_for_mednist_dataset() -> str:
    ...
    while ask_user:
        val = input(
            "How many samples do you want to load? (press enter for the full dataset)\n"
        )
        if val == "":
            ask_user = False
            ...
            print(
                f"Number of samples exceed size of dataset or below minimum (6 samples at least) (asked {val}!)"
            )
...
class MedNISTDataset(DatasetManager):
    ...
    def sample_image_dataset(
        ...
        new_image_folder_path = os.path.join(
            Path(self._folder_path).parent, new_sampled_dataset_name
        )
        _do_sampling = True
        if os.path.exists(new_image_folder_path):
            ...
            _do_sampling = (
                input("Do you want to delete existing dataset (y/n)").lower() == "y"
            )
        ...
        if _do_sampling:
            ...
            for i, directory in enumerate(self.directories):
                ...
                for image_path in images_path[self._old_idx[i] : _idx_max]:
                    shutil.copy2(
                        os.path.join(label_img_path, image_path),
            ...
            while rest > 0:  # here rest < n_classes
                ...
                shutil.copy2(
                    os.path.join(self._folder_path, directory, image_path),
    ...
    for n in range(abs(n_nodes)):
        ...
        if n_sample != "":
            ...
            d_path = dataset.sample_image_dataset(
                int(n_sample), n_classes=6, new_sampled_dataset_name=_name
            )
        else:
