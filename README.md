...
```
$ fedbiomed node --directory ./my-second-node start
```
...
```
$ RESEARCHER_SERVER_HOST=192.168.0.100 fedbiomed node start
$ RESEARCHER_SERVER_HOST=192.168.0.100fedbiomed researcher start
```
...
```shell
ENABLE_TRAINING_PLAN_APPROVAL=True ALLOW_DEFAULT_TRAINING_PLANS=True fedbiomed node -d my-node start
```
...
```shell
ENABLE_TRAINING_PLAN_APPROVAL=False ALLOW_DEFAULT_TRAINING_PLANS=False fedbiomed node -d my-node start
```
...
```shell
fedbiomed node [--directory [COMPONENT_DIRECTORY]] gui --data-folder '<path-for-data-folder>' start
```
...
- ``--directory``: Component directory  whose GUI will be launched which is going to be used for GUI. If it is not
...
```shell
fedbiomed node --directory <path/to/component/directory> gui --data-folder '<path-for-data-folder>' ' cert '<name-of-certificate>' key '<name-of-private-key>' start
```
