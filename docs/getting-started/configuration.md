...
For more detailed configuration instructions, please refer to the [Configuring Nodes]() and [Deployment]() user guides.
...
All the actions that are specific to Node component should be declared after the option `node` of `fedbiomed` command. For example, `fedbiomed node dataset list` will list all the datasets deployed on the dataset. You can list all possible options and action by executin `fedbiomed node --help`.
