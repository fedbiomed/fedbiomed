...
Start the researcher first, then the nodes. The researcher starts even with no node
certificate registered yet — it binds the port and rejects every node handshake until
a certificate is registered, which it then picks up without a restart. On success you
will see security log lines such as:
...
| Debug `Researcher server is not available` (node retries) | The endpoint is not up | Check the researcher started |
