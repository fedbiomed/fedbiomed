...
```
pip install git+https://github.com/fedbiomed/fedbiomed.git["gui"]
```
...
Node GUI can be started through `fedbiomed`  command with various settings such as ip, port, folder where data files
are stored or the configuration that specifies the node that the GUI will run for.

The following command is the basic command to start the Node GUI with default settings. It uses the default Fed-BioMed Node component directory, which corresponds to the directory where the command is executed. If no Node component is instantiated in that directory, a new one will be automatically created.

Unless it is specified, this command assumes that data files are stored in the data directory within the Node component folder. For example, if the command is executed in `/path/to/workdir`, the Node component will be instantiated in `/path/to/workdir/fbm-node/`, with the default data directory located at `/path/to/workdir/fbm-node/data`.
...
It is possible to specify the node that the user interface will be used for through the option `--directory` or `-d`.

````
$ fedbiomed node gui --directory <path/to/component/directory> start
````
...
```shell
$ fedbiomed node gui start --port 5001 --directory ./my-first-node start
$ fedbiomed node gui start --port 5002 --directory ./my-second-node start
$ fedbiomed node gui --port 5003 --directory ./my-third-node start
```
