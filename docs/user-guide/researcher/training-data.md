...
A user is also allowed to define a [`CustomDataset`](../datasets/custom-dataset.md), where the user directly writes how to `read` and `get_item` from the dataset. In this case, it is upto the user to whether to pass the targets through the `dataset` object, `target` variable or to not give targets (unsupervised) at all.
