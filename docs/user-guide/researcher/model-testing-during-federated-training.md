...
!!! info "Validation and training dataset are kept separated when testing is enabled"
...
!!! warning "Validation dataset are re-shuffled if `test_ratio` changes or if `Node's` dataset change from one `Round` to another"
    ...
    Similarly, if changes in the `Node` dataset happens form one `Round` to another, dataset training and validation may change as well. 
