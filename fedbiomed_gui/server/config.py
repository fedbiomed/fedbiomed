...
class Config(dict):
    ...
    def generate_config(self):
        ...
        # Data path ----------------------------------------------------------------
        data_path = os.getenv(
            "DATA_PATH", cfg.get("server", "DATA_PATH", fallback="data")
        )

        if data_path.startswith("/"):
            assert os.path.isdir(data_path), (
                f'Data folder path "{data_path}" does not exist or it is not a directory.'
            )
        else:
            data_path = os.path.join(
                self.configuration["NODE_FEDBIOMED_ROOT"], data_path
            )
            assert os.path.isdir(data_path), (
                f"{data_path} has not been found in Fed-BioMed root directory or "
        ...
        self.configuration["DEFAULT_ADMIN_CREDENTIAL"] = {
            "email": cfg.get("init_admin", "email"),
            "password": cfg.get("init_admin", "password"),
        }

        # Get name of the config file default is "config_node.ini"
        self.configuration["NODE_CONFIG_FILE"] = os.getenv(
            "NODE_CONFIG_FILE", "config.ini"
        )
        ...
        print(
            f"INFO: Fed-BioMed Node root dir has been set as "
            f"{self.configuration['NODE_FEDBIOMED_ROOT']} \n"
        )

        print(
            f"INFO: Services are going to be configured for the node "
