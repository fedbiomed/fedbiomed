    ...
    if dataset:
        ...
        if matched:
            dataset["path"] = dataset["path"].replace(
                config["NODE_FEDBIOMED_ROOT"], "$FEDBIOMED_DIR"
            )
...
def add_default_dataset():
    ...
    if dataset:
        return error(
            f"Default dataset has been already deployed with tags: {req['tags']}"
        ), 400
    ...
    if "path" in req:
        ...
        if not os.path.isdir(path):
            return error(
                "Provided path is not a directory. Please select a folder not a file."
            ), 400
    else:
...
def get_csv_data():
    ...
    if not os.path.isfile(data_path):
        return error(
            f"Path does not correspond to a valid data file: {os.path.join(*req['path'])}"
        ), 400
    ...
    try:
        ...
        return error(
            f"Can not read given data file please make sure the format "
            f"is one of csv, tsv or txt: {e}"
        ), 400
