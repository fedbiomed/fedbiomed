...
def list_data_path():
    ...
    if os.path.exists(dpath) and os.path.isdir(dpath):
        ...
        all_datasets = dataset_manager.list_my_data()
            ...
            if not file.startswith("."):
                ...
                if not dataset and os.path.isdir(fullpath):
                    includes = [
                        ...
                        if re.search(
                            "^" + os.path.join(fullpath, ""), d.get("path", "") or ""
                        )
                    ]
        ...
        return error(
            f"Requested path does not exist or it is not a directory. {req_path}"
        )
