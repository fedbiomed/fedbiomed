...
def register():
    ...
    if req["confirm"] != req["password"]:
        return error("Password confirmation does not match to the password"), 400
    ...
    return response(
        ...
        "A request has been sent to administrator for account creation",
    ), 201
...
def login():
    ...
    if not user:
        ...
        return error(
            f"Unrecognized email address {email}. Please register before to log in"
        ), 401
...
def refresh_expiring_jwts():
    ...
    access_token = create_access_token(
        identity=jwt["sub"], additional_claims=additional_claims, fresh=False
    )
    refresh_token = create_refresh_token(
        identity=jwt["sub"], additional_claims=additional_claims
    )
