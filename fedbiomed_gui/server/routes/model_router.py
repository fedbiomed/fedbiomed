...
def list_training_plans():
    ...
    res = TP_SECURITY_MANAGER.list_training_plans(
        sort_by=sort_by, select_status=select_status, verbose=False, search=search
    )
...
def preview_training_plan():
    ...
    if res is None:
        return error(
            f"No training plan with provided training plan id {training_plan_id} found in database"
        ), 400
    else:
        return response(res), 200
