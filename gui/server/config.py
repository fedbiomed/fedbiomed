...
class Config(dict):
    ...
    def generate_config(self):
        ...
        if not os.path.isfile(conf):
            default_config = os.path.join(ROOT_DIR, 'gui', 'config_gui.ini')
        ...
        self.configuration['FBM_NODE_CONFIG_FILE_PATH'] = os.path.join(
            self.configuration["NODE_FEDBIOMED_ROOT"], 'etc', 'config.ini'
        )

        node_config = NodeConfig(root=self.configuration["NODE_FEDBIOMED_ROOT"])
        node_id = node_config.get('default', 'id')
        self.configuration['ID'] = node_id

        # Set DB_PATH based on given node id
        self.configuration['NODE_DB_PATH'] = os.path.join(
            self.configuration["NODE_FEDBIOMED_ROOT"], 'etc', node_config.get('default', 'db')
        )
